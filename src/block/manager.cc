...
namespace chfs
{
      ...
      -> ChfsNullResult
  {
    ...
    if (block_id < 0 || block_id >= this->block_cnt || data == nullptr)
    {
      std::cout << "1 return ChfsNullResult(ErrorType::INVALID_ARG)" << std::endl;
      return ChfsNullResult(ErrorType::INVALID_ARG);
    }

    auto offset = static_cast<usize>(block_id) * this->block_sz;
      ...
      -> ChfsNullResult
  {
    ...
    usize block_offset = static_cast<usize>(block_id) * this->block_sz;
    usize block_end = block_offset + this->block_sz;

    if (block_offset + offset + len > block_end)
    {
      return ChfsNullResult(ErrorType::INVALID_ARG);
    }
  ...
  auto BlockManager::read_block(block_id_t block_id, u8 *data) -> ChfsNullResult
  {
    ...
    usize offset = static_cast<usize>(block_id) * this->block_sz;
  ...
  auto BlockManager::zero_block(block_id_t block_id) -> ChfsNullResult
  {
    ...
    if (block_id < 0 || block_id >= this->block_cnt)
    {
      return ChfsNullResult(ErrorType::INVALID_ARG);
    }
